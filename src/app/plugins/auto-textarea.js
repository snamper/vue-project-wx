        ...
        Vue.directive("autoheight",{
            ...
            bind(el,binding){
                setTimeout(function () {
                    ...
                    $('body').append("<div class='hidebox2'></div>")
                    ...
                    $('.hidebox2').height($(el).height())
                    //$('.hidebox').css('whiteSpace','pre')
                })
                el.oninput=function () {
                    var Height = $(el).css('line-height').replace('px','')
                    var reg=new RegExp("\n","g");
                    var str
                    setTimeout(function () {
                        console.log(el.value.match(reg))
                        str=el.value.match(reg)?el.value.match(reg).length:0
                        console.log(str)
                        if(el.value){
                            $('.hidebox').text(el.value)
                            // $(this).height($('.hidebox').height())
                            if(str>0){
                                $(el).height($('.hidebox').height()+Height*str)
                            }else{
                                $(el).height($('.hidebox').height())
                            }
                            console.log(Height*str,$('.hidebox').height())
                            console.log($(this).height())
                        }else{
                            $(el).height($('.hidebox2').height())
                        }
                    })
