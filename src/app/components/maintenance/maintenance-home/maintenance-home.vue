<template>
    ...
    <div class="home" :class="{'noscroll':changeShow || searchShow}">
        ...
        <section v-show='loadData && brandList.length!=0' >
            ...
            <header class='header flex' v-bind:class="{'filter':searchShow,'hide222':changeShow}">
                ...
                <div class='feedback' @click='feedback()'>
                    <i class='iconfont  font-16 icon-woshenpideline_'>&#xe732</i>
                    <i class=' icon anticon icon-stepforward' ></i>
                    <span class='font-9'>反馈</span>
                </div>
            </header>
            <div class='main'
                 v-bind:class="{'filter':searchShow,'hide222':changeShow,'noscroll':changeShow!=''}">
                <div class='content p-t-9' >
                    <div class='hot'>
                        <div class='font-16 color-gray2 title'>
                            <span>热门品牌</span>
                        </div>
            ...
            <transition>
                <div class='changeTime' :class="{'height0':!changeShow,'transition':isiOS}" >
                    <div class='car-name font-11 flex'>
                        <div class='left flex'>
                            <img :src="currentChoosed.logo" alt="">
                            <div class='color-gray5 text' v-if='changeShow=="changBrand"' v-text='currentChoosed.brand'>
                                安驰</div>
                            <div v-if='changeShow=="changeType"'>
                                <span class='ddid21 text'  v-text='moment(currentChoosed.brand)' @click='back(1)'>安驰
                                </span><i
                                class='iconfont icon-arrowR font-10 color-grayC'></i><span class='color-gray5'
                                                                                           v-text='currentChoosed.name'>安驰</span>
                            </div>
                            <div v-if='changeShow=="changeTime"'>
                    ...
                    <div class='car-time font-14 color-gray2'>
                        <div class='time-item flex' v-if='changeShow=="changBrand"' v-for='item in
                        currentChoosedNameList' @click='choose(2,item)'>
                            <span v-text='item'>威宝</span>
                            <i class='iconfont icon-arrowR font-10 color-grayC'></i>
                        </div>
                        <div class='time-item flex' v-if='changeShow=="changeType"' v-for='item in
                        currentChoosedDisplacementList'
                             @click='choose(3,item)'>
                            <span>{{item.salesName}}</span>
                            <i class='iconfont icon-arrowR font-10 color-grayC'></i>
                        </div>
                        <div class='time-item flex' v-if='changeShow=="changeTime"' @click='choose(4,item)'
                             v-for='item in currentChoosedYearList'>
            ...
            <!--<div class='changeTime' v-show='changeShow=="changeType"' :class="{'height0':changeShow==''}">-->
            ...
            <!--</div>-->
            <!--</div>-->
            <div class='searchPage' v-bind:class="{'opacity':searchShow&&!changeShow,'hide222':changeShow,}" v-show='searchShow'>
                <div class='search-default font-13 color-gray9'>
                    <div class='searchBox' @click="searchDefault=true">
                        ...
                        <div class='search flex'>
                            ...
                            <i class='iconfont icon-quxiaoglyph_ font-13' @click='searchName="";searchDataList=[]'></i>
...
//                })
            ...
            choose(index, current) {
                ...
                if (index == 2) {
                    ...
                    self.currentChoosedDisplacementList=''
                ...
                if (index == 3) {
                    ...
                    self.currentChoosed.type = current.salesName
                    self.currentChoosedYearList=''
                    api.chooseYear({
                        brandName: self.currentChoosed.brand,
                        models: self.currentChoosed.name,
                        salesName: self.currentChoosed.type,
                    }).then((data) => {
                        if (data.result_code == 0) {
            ...
            Photograph(){
                ...
                chooseImage()
                    ...
                    .then((serverId)=>(
                        setTimeout(function () {
                            loading.show('加载中...')
                            api.getVinByImg({vinImgId: serverId}).then((data) => {
                                loading.show('加载中...')
                                if (data.result_code == 0) {
                                    self.levelId = data.response.levelId
                                    self.$router.push({path:'/maintenance/maintenance-recommend',query: {levelId:self.levelId}});
            ...
            scroll(){
                ...
                $(document).scroll(function(event){
                    ...
                    for(var i in self.brandList){
                        //  console.log($('.brand-item').eq(1).offset().top-$(document).scrollTop())
                        if($('.brand-item').eq(i).offset().top-$(document).scrollTop()<47){
